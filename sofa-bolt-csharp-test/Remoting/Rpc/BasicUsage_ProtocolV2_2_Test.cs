...
namespace com.alipay.remoting.rpc
{
    ...
    public class BasicUsage_ProtocolV2_2_Test: IDisposable
    {
		private bool InstanceFieldsInitialized = false;

		public BasicUsage_ProtocolV2_2_Test()
		{
		...
		{
			addr = "127.0.0.1:" + port + "?_PROTOCOL=2&_VERSION=2";
		}
...
//ORIGINAL LINE: final java.util.concurrent.CountdownEvent latch = new java.util.concurrent.CountdownEvent(1);
				...
				try
				{
					if (i % 2 == 0)
					{
						client.invokeWithCallback(addr, req, new InvokeCallBackImpl(this, rets, latch), 1000);
					}
					else
					{
						InvokeContext invokeContext = new InvokeContext();
						invokeContext.putIfAbsent(InvokeContext.BOLT_CRC_SWITCH, false);
						client.invokeWithCallback(addr, req, invokeContext, new InvokeCallBackImpl(this, rets, latch), 1000);
					}
				}
				catch (RemotingException e)
				{
		...
		private class InvokeCallBackImpl : InvokeCallback
		{
			private readonly BasicUsage_ProtocolV2_2_Test outerInstance;

			internal Executor executor = Executors.newCachedThreadPool();
			internal IList<string> rets;
			internal CountdownEvent latch;

			internal InvokeCallBackImpl(BasicUsage_ProtocolV2_2_Test outerInstance, IList<string> rets, CountdownEvent latch)
			{
